...
function showC3LineChart() {
  ...
  d3.json('http://localhost:5000/data/count', function(error2, result2) {
    result2 = result2['data'];
    console.log(result2);
    dates2 = ['dates'];
    counts2 = ['counts'];
    for (date2 in result2) {
      dates2.push(date2);
      counts2.push(result2[date2]);
    }
    var chart2 = c3.generate({
        bindto: '.c3-line-chart-holder',
        data: {
          x: 'dates',
          columns: [ dates2, counts2 ]
        },
        axis: {
            x: {
                type: 'timeseries',
...
function showHeatmap() {
  d3.json('http://localhost:5000/data/heatmap', function(error, result) {
    ...
    for (var floor in result) {
          ...
          var svg = d3.select("#heatmap-holder").append("svg")
              .attr("width", width + margin.left + margin.right)
              .attr("height", height + margin.top + margin.bottom)
              .append("g")
              .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
